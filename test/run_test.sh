...
run_test_file() {
  IFS='|' read COMMAND OPTIONS DOC_NAME INPUT EXPECTED_EXIT < $TEST
  COMMAND=`echo $COMMAND`               # trims whitespace
  DOC_NAME=`echo $DOC_NAME`             # trims whitespace
  ...
  then                                  # success: diff output file
    ...
    then
      ...
      EXPECTED_BIN="$EXPECTED_DIR/`echo $TEST_NAME | sed 's/test$/pdb/'`"
      if [ -f $EXPECTED_TXT ]
